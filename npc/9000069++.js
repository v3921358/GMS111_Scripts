...
function action(mode, type, selection) {
	...
	if (status == 2) {
	    cm.sendOk("這是一件嚴重的事情！如果你不願意做，就別打擾我。");
	    cm.safeDispose();
	    return;
    ...
    if (status == 0) {
	cm.sendNext("卷軸始終被視為Mapler設備陞級過程的重要方面。你是那些努力在手工資料上達到完美的人之一嗎?#b\r\n#L0#Create a 65% Scroll#l\r\n#L1#Create a 15% scroll#l#k");
    } else if (status == 1) {
	firstSel = selection;
	cm.sendYesNo("哦，你好。你似乎比我更感興趣。我可以隨時陞級你的卷軸，只需一點費用和一些資料。你確定要這麼做嗎？#bThere is a 10% 在創作過程中卷軸可能被破壞的可能性.#k")
    } else if (status == 2) {
	cm.sendNext("下麵是古代黑魔法卷軸復活的資料. \n\r #i4011006# #t4011006# x1 \n\r #i4021007# #t4021007# x1 \n\r #i4021008# #t4021008# x1 \n\r Mesos 500,000 \n\r\n\r Lastly, it will also require the 10% scroll.");
    } else if (status == 3) {
	var message = "這是我許可權範圍內要創建的卷軸清單 : \n\r";
if (firstSel == 0) {
	...
	if (firstSel == 0) {
	    ...
	    if (prompt == -2) {
		cm.sendOk("看來您缺少50萬個mesos。");
	    } else if (prompt == -3) {
		cm.sendOk("看來你沒有合適的使用空間。");
	    } else {
		cm.sendOk("看來你缺少足够的錢 #i"+prompt+"# #t"+prompt+"#.");
	    }
	    cm.safeDispose();
	}
    }
