...
function action(mode, type, selection) {
	...
	cm.dispose();
	if (status == 0 && mode == 1) {
		var selStr = "你好，我住在這裡，但不要低估我。 我幫你做一雙新鞋怎麼樣？#b"
	...
	if (status == 1 && mode == 1) {
		...
		if (selectedType == 1){ //bowman shoe
			selStr = "你想要做的弓箭手什麼鞋子？#b";
		...
		if (selectedType == 2){ //magician shoe
			selStr = "你想要做的法師什麼鞋子？#b";
		...
		if (selectedType == 3){ //thief shoe
			selStr = "你想要做的盜賊什麼鞋子？#b";
	...
	if (status == 3 && mode == 1) {
		...
		if (!complete)
			cm.sendOk("很抱歉由於你的材料不足。 也許下次才能帮到你吧.");
		else {
		if (mats instanceof Array) {
			for (var i = 0; i < mats.length; i++){
