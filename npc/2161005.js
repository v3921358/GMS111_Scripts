...
function start() {
	...
	status = 0;
		if (time + (12 * 3600000) >= cm.getCurrentTime() && !cm.getPlayer().isGM()) {
			cm.sendOk("在過去的12小時裏妳已經挑戰過獅子王，剩餘冷卻時間: " + cm.getReadableMillis(cm.getCurrentTime(), time + (12 * 3600000)));
			cm.dispose();
			return;
			}
			cm.sendYesNo("#e<探險：獅子王>#n\r\n\r\n\r\n現在可以申請獅子王探險隊，你想成為探險隊隊長嗎？");
	...
	case 0:
		if (mode == 1) {
			if (cm.registerSquad("VonLeon", 5, " 已經成為了<獅子王>探險隊隊長，如果你想嘗試本次探險，請重新與我對話申請登記探險.否則你將無法參與本次探險。")) {
				cm.sendOk("已經成為<獅子王>探險隊隊長，請在5分鐘內召集好探險隊隊員進行探險.否則將會自動註銷本次探險資格。");
			} else {
