...
function action(mode, type, selection) {
	...
	cm.dispose();
	if (status == 0 && mode == 1) {
		var selStr = "我大部分時間都在這裡工作，為像你這樣的旅行者製造武器。 你有什麼要求嗎？#b"
		var options = new Array("什麼是催化劑?", "做一雙劍士武器", "做一雙弓箭手武器", "做一雙法師武器", "做一雙盜賊武器",
		"做一雙劍士武器使用催化劑", "做一雙弓箭手武器使用催化劑", "做一雙法師武器使用催化劑", "做一雙盜賊武器使用催化劑");
	...
	if (status == 1 && mode == 1) {
		...
		if (selectedType == 4){  //thief weapon; claws vary depending if stimulator is being used
